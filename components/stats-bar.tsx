...
const stats = [
  ...
  { value: 2500, suffix: "+", label: "רוכשים מרוצים בארץ ובעולם" },
  { value: 50, suffix: "+", label: "פרויקטים שמותגו ושווקו" },
]
...
export function StatsBar() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-border/50 bg-card/40 py-14 md:py-16">
      ...
      <div ref={ref} className="mx-auto grid max-w-5xl grid-cols-1 gap-10 px-5 sm:grid-cols-3 sm:gap-6 md:px-8">
        {stats.map((s, i) => (
          <div key={s.label} className="relative flex justify-center" style={run ? { animation: `count-in 0.6s ease ${i * 120}ms both` } : { opacity: 0 }}>
